...
function Sidebar({
  ...
  return (
    <aside className="w-64 shrink-0 h-full overflow-y-auto border-r border-fern/20 bg-snow/50 p-4 flex flex-col">
      {/* 新建按钮 */}
      <div className="flex items-center gap-2 mb-4">
        <button
          onClick={() => { onNew(); onClose?.(); }}
          className="flex-1 py-2.5 bg-evergreen text-snow rounded-full text-sm font-medium
        ...
        {onClose && (
          <button
            ...
            className="md:hidden w-9 h-9 flex items-center justify-center rounded-lg
              text-sage hover:bg-mist transition-colors"
          >
            ✕
          </button>
        )}
            ...
            >
              ...
              <button
                ...
                className="ml-2 text-sage/30 hover:text-amber opacity-0 group-hover:opacity-100
                  transition-all text-xs shrink-0 p-1 min-w-[28px]"
                title="删除"
              >
                ✕
              </button>
...
export default function ChatPage() {
  ...
  return (
    <div className="flex h-[calc(100dvh-7rem)] md:h-[calc(100vh-4rem)]">
      {/* Desktop sidebar */}
      ...
      {sidebarOpen && (
        <div className="md:hidden fixed inset-0 z-40 flex">
          ...
          />
          <div className="relative z-10 h-full animate-[slideIn_200ms_ease-out]">
            <Sidebar
              conversations={convList}
              activeId={conversationId}
              loading={convLoading}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Mobile top bar */}
        <div className="md:hidden flex items-center gap-2 px-4 py-2 border-b border-fern/20">
          <button
            onClick={() => setSidebarOpen(true)}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-mist text-xs text-shadow
              hover:bg-fern/20 transition-colors min-h-[36px]"
          >
            💬 对话列表
          </button>
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 sm:py-6 space-y-3 sm:space-y-4">
          {messages.length === 0 && (
            <div className="text-center text-sage mt-16 sm:mt-20">
              <p className="font-serif text-xl sm:text-2xl text-shadow mb-3">
                🍃 今天想去哪里旅行？
              </p>
              <p className="text-sm">
          ...
          {messages.map((msg, i) => (
            ...
            >
              <div
                className={`max-w-[85%] sm:max-w-[75%] px-3.5 sm:px-4 py-2.5 sm:py-3 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap
                  ${msg.role === "user"
        ...
        {/* 输入栏 */}
        <div className="border-t border-fern/50 px-3 sm:px-6 py-3 sm:py-4 bg-mist/50">
          <div className="flex gap-2 sm:gap-3">
            ...
            <button
              ...
              className="px-5 sm:px-6 py-3 bg-evergreen text-snow rounded-full font-medium text-sm
